...
s1_sub_a = [
    ...
    SubOption("social", "마을 주민들과 AI가 함께 공공 건물에 난민을 분산 배치하는 '하람비' 구호 계획을 수립하고, 장기적인 화합 및 외부 지원 요청 방안을 AI에게 재학습시킨다.", "AI의 단기적 계산보다 공동체 조화와 장기적 생존을 위한 인간의 사회적 책임을 강조한다."),
...
SCENARIOS: List[Scenario] = [
    Scenario(
        ...
        setup="당신은 아프리카 해안 마을에 시범 도입된 '공동체 생존 및 자원 관리 AI 시스템 (Harambell-S-1)'의 책임 운영자입니다."
    ...
    Scenario(
        ...
        setup="당신은 서아프리카 내전 속에서 70명의 피난민을 이끄는 리더입니다. 생존 확률을 극대화하기 위해 부족원들은 '생존 분석 AI (E-Tribe)'를 신뢰하고 있습니다. 이 AI는 **최대 다수의 생존**을 목표로 설계되었으며, 부족 단위의 강한 정체성('우리는 한 몸')은 데이터로 입력된 상태입니다. 현재 갈림길에서 AI는 두 경로를 분석하여 리더인 당신에게 결정을 요구합니다.",
        main_options={
    ...
    Scenario(
        ...
        setup="당신은 사헬 지대 '키주투 마을'의 유일한 수입원인 망가 탄광의 베테랑 현장 관리자입니다. 탄광에는 붕괴 위험 감지 및 대응을 위한 '탄광 안정화 AI' 시스템이 시범 운영 중입니다. AI는 심각한 붕괴를 감지했으나, 최적의 해답을 제시하지 못하고 최종 결정 레버를 당신에게 넘겼습니다. 마을 전체 인구는 약 50명이며, 이는 5명의 인부와 나머지 45명으로 구성됩니다.",
        main_options={
        ...
        base_stats={
            "A": {"lives_saved": 45, "lives_harmed": 5, "risk": 0.5}, #5명 희생 (10%) vs 45명 생존 (90%)
            "B": {"lives_saved": 5, "lives_harmed": 45, "risk": 0.95} # 45명 희생 (90%) vs 5명 생존 (10%)
        }
    )
]
